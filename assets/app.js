...
function findMunicipality(address){
  const rawCode=String(address.gemeentecode||address.gemeente_code||"").replace(/^GM/i,"").replace(/\D/g,"");
  if(rawCode){const gm=`GM${rawCode.padStart(4,"0")}`;const byCode=state.municipalities.find(x=>x.code===gm);if(byCode)return byCode;}
  const aliases={sgravenhage:"Den Haag",gravenhage:"Den Haag"};
  const source=normalizeMunicipalityName(address.gemeentenaam);
  const target=aliases[source]||address.gemeentenaam;
  return state.municipalities.find(x=>normalizeMunicipalityName(x.name)===normalizeMunicipalityName(target));
}
...
load().catch(error=>{el("message").textContent="De gegevens konden niet worden geladen. Probeer de pagina opnieuw te laden.";el("message").className="error";console.error(error)});
