...
function renderRule(r,e){const [label,klass]=publicState(e),application=r.officialApplicationUrl?link(r.officialApplicationUrl,"Start aanvraag","application-cta"):'<span>Aanvraagroute nog niet bevestigd</span>';return `<article class="card"><span class="result-state ${klass}">${label}</span><h3>${esc(r.title)}</h3><p>${esc(e.reason)}</p>${renderQuestions(r,e)}<details><summary>Voorwaarden, documenten en officiële links</summary><p>${esc(r.shortDescription)}</p><div class="result-links"><div><h4>Meer informatie</h4>${link(r.officialInformationUrl,"Bekijk gemeentelijke uitleg")}</div><div><h4>Officiële regeling</h4>${link(r.officialRegulationUrl,"Bekijk regeling")}</div><div><h4>Vergunning aanvragen</h4>${application}</div></div></details></article>`}
