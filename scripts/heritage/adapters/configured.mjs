...
function addressOf(r,c){const direct=first(r,c.addressFields||[]);if(direct)return norm(direct);const street=norm(first(r,c.streetFields||['straat','straatnaam','Straatnaam'])),number=norm(first(r,c.numberFields||['huisnummer','Huisnummer'])),addition=norm(first(r,c.additionFields||['toevoeging','Toevoeging'])),postcode=norm(first(r,c.postcodeFields||['postcode','Postcode'])).replace(/\s+/g,'').toUpperCase();return [street,[number,addition].filter(Boolean).join(' '),postcode].filter(Boolean).join(', ')}
