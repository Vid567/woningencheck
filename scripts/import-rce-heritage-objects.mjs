...
for (const municipality of municipalities) {
  for (const name of [municipality.name, municipality.canonicalName, ...(municipality.aliases || [])].filter(Boolean)) byName.set(norm(name), municipality.code);
}

const monumentIdFields = ['OBJ_RIJKSNUMMER', 'monumentnummer', 'monumentnr', 'monument nummer', 'monument_id', 'monumentid', 'rijksmonumentnummer', 'objectnummer'];
const objectIdFields = ['OBJ_NUMMER', 'object_id', 'object id', 'objectid', 'id_object', 'id object', 'idobject', 'objectidentificatie', 'object identificatie'];
const fields = {
...
const normalizedFields = Object.fromEntries(Object.entries(fields).map(([key, aliases]) => [key, aliases.map(norm)]));
...
async function* csvRecords(file) {
  ...
  let row = [], value = '', quoted = false, headers = null;
  const emitRow = raw => {
    if (!headers) { headers = raw.map(norm); return null; }
  ...
  for await (const chunk of stream) {
    for (let i = 0; i < chunk.length; i++) {
      const char = chunk[i];
      if (quoted) {
        if (char === '"') { if (chunk[i + 1] === '"') { value += '"'; i++; } else quoted = false; }
        else value += char;
      } else if (char === '"') quoted = true;
      else if (char === ',') { row.push(value); value = ''; }
      else if (char === '\n') { row.push(value.replace(/\r$/, '')); value = ''; const record = emitRow(row); row = []; if (record) yield record; }
      else value += char;
    }
  }
  if (value || row.length) { row.push(value.replace(/\r$/, '')); const record = emitRow(row); if (record) yield record; }
}

const makeAggregate = (objectId, monumentNumber) => ({ objectId, monumentNumber, tables: new Set(), values: Object.fromEntries(Object.keys(fields).map(key => [key, new Set()])) });
function absorb(aggregate, row, table) {
  aggregate.tables.add(table);
  for (const [key, aliases] of Object.entries(normalizedFields)) { const value = pick(row, aliases); if (value) aggregate.values[key].add(value); }
}
...
const byObjectId = new Map(), byMonumentNumber = new Map();
let objectRows = 0, objectRowsWithoutId = 0, objectRowsWithoutMonument = 0;
for await (const row of csvRecords(path.join(input, objectTable))) {
  objectRows++;
  const objectId = pick(row, normalizedObjectIdFields), monumentNumber = pick(row, normalizedMonumentIdFields);
  if (!objectId) { objectRowsWithoutId++; continue; }
  if (!/^\d+$/.test(monumentNumber)) { objectRowsWithoutMonument++; continue; }
  let aggregate = byObjectId.get(objectId);
  if (!aggregate) { aggregate = makeAggregate(objectId, monumentNumber); byObjectId.set(objectId, aggregate); byMonumentNumber.set(monumentNumber, aggregate); }
...
let relatedRows = 0, joinedRows = 0, unjoinedRows = 0;
const tableStats = {};
for (const file of files) {
  if (file === objectTable || file.startsWith('idx') || file.startsWith('sys')) continue;
  let rows = 0, joined = 0, unjoined = 0;
  for await (const row of csvRecords(path.join(input, file))) {
    ...
    if (!aggregate) { unjoined++; unjoinedRows++; continue; }
    absorb(aggregate, row, file); joined++; joinedRows++;
  }
...
function buildRecord(aggregate) {
  ...
  const names = join(values.names), streets = join(values.streets), numbers = join(values.numbers), postcodes = join(values.postcodes), places = join(values.places);
  const addresses = join(values.addresses);
  const composedAddress = [streets[0], numbers[0], postcodes[0], places[0]].filter(Boolean).join(' ').trim();
  if (composedAddress && !addresses.some(value => norm(value) === norm(composedAddress))) addresses.push(composedAddress);
  ...
  const municipalityCode = digits.length >= 4 ? `GM${digits.slice(-4)}` : byName.get(norm(municipalityName)) || null;

  const type = join(values.types).join(' '), functie = join(values.functions).join(' '), omschrijving = join(values.descriptions).join(' '), name = names[0] || null;
  const classification = classifyHeritageObject({ type, functie, name, omschrijving });
  const bagPandIds = join(values.bagPandIds), bagAddressIds = join(values.bagAddressIds);
  return {sourceId:'rce-extract-mrs',sourceRecordId:aggregate.monumentNumber,monumentNumber:aggregate.monumentNumber,municipalityCode,bagPandIds,bagAddressIds,addresses,objectType:classification.objectTypes[0]||null,objectTypes:classification.objectTypes,heritageType:'rijksmonument',designationStatus:'designated',matchMethod:bagPandIds.length||bagAddressIds.length?'bag_relation':addresses.length?'address_exact':'source_assertion',name,officialUrl:`https://monumentenregister.cultureelerfgoed.nl/monumenten/${aggregate.monumentNumber}`,checkedAt:today,objectTypeClassification:{method:classification.method,confidence:classification.confidence,taxonomyVersion:1},raw:{type,functie,omschrijving,extractTables:[...aggregate.tables]}};
}
...
let total = 0, classified = 0, addressed = 0, withMunicipality = 0;
const counts = {};
let first = true;
for (const aggregate of byObjectId.values()) {
  const record = buildRecord(aggregate); total++;
...
const status = {schemaReady:true,resolverReady:true,classifierReady:true,nationwideObjectImportPending:false,nationalRceImportComplete:true,municipalObjectImportCoverage:'separate-source-adapters',recordCount:total,classifiedObjectCount:classified,addressCount:addressed,municipalityCodeCount:withMunicipality,note:'Volledige landelijke Rijksmonumentenregister-import uit officiële RCE Monumentendatabank Extract_MRS.'};
await writeChunk(`\n  ],\n  "status": ${JSON.stringify(status, null, 2).replace(/^/gm, '  ').trimStart()}\n}\n`);
writer.end(); await once(writer, 'close');

await fs.writeFile('data/heritage-object-import-report.json', JSON.stringify({generatedAt:new Date().toISOString(),source:'RCE Monumentendatabank Extract_MRS',officialLandingPage:'https://www.cultureelerfgoed.nl/onderwerpen/r/rijksmonumentenregister/monumentendatabank',objectTable,objectIdField:'OBJ_NUMMER',monumentNumberField:'OBJ_RIJKSNUMMER',tables:files,objectRows,total,classified,unclassified:total-classified,withAddress:addressed,withMunicipality,relatedRows,joinedRows,unjoinedRows,tableStats,objectTypeCounts:counts,completeNationwideDump:true,importerMode:'streaming-object-id-join'}, null, 2) + '\n');
