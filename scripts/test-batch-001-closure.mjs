...
if(batch.closure?.remainingHumanItems!==3)fail('Batch 001 must close with exactly three targeted human items');
const openBatch=reviews.filter(x=>x.id.startsWith('batch001-')&&['open','in-review'].includes(x.status));
if(openBatch.length!==3)fail(`Expected three open Batch 001 reviews, got ${openBatch.length}`);
...
if(params.length!==5)fail('Expected five tracked annual parameters');
