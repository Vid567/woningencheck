...
const records=JSON.parse(fs.readFileSync("data/regulations.json","utf8")).records;for(const r of records.filter(x=>x.geographicScope)){const raw=JSON.stringify(r.geographicScope);assert(raw.includes(r.municipalityCode),r.id+" geography lacks municipality-specific evidence");}console.log("PASS geography: municipality, postcode, street, range, district, neighbourhood, polygon, AND/OR, property, unknown map and cross-municipality evidence");

const almelo=JSON.parse(fs.readFileSync('data/batch-001-geographic-scopes.json','utf8')).scopes[0].scope;
