...
test('homepage houdt uitleg voorop en verwijst naar aparte adrescheck',async({page})=>{await page.goto('/');await expect(page.locator('#postcode')).toHaveCount(0);const link=page.getByRole('link',{name:/adres|check/i}).filter({has:page.locator('text=/./')}).first();await expect(link).toHaveAttribute('href',/adrescheck\.html/)});
